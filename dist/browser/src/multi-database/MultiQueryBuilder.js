...
export class MultiQueryBuilder {
    ...
    get() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof this.paginateLimit === 'number' && typeof this.paginateSkip === 'number') {
                const countResults = yield Promise.all(dbNames.map(db => QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db).setIsMultiDatabase(false).setQueries(query).count()));
                ...
                const results = yield Promise.all(dbs.map((db) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const result = yield QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db.config.dbName).setPeriod(db.period).setIsMultiDatabase(false).setQueries(query).paginate(skip, limit).get();
                    limit -= result.length;
                    skip = 0;
                    return result;
                })));
            ...
            const results = yield Promise.all(dbs.map((db) => __awaiter(this, void 0, void 0, function* () {
                const result = yield QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db.config.dbName).setPeriod(db.period).setIsMultiDatabase(false).setQueries(query).get();
                return result;
            })));
            return results.flat();
        });
    ...
    count() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const query = this.getQuery();
            const counts = yield Promise.all(dbs.map(db => QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db).setIsMultiDatabase(false).setQueries(query).count()));
    ...
    create(attributes, currentPeriod = moment().format('YYYY-MM'), fallbackCreate = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const dbs = yield this.getDbs();
            let db = dbs.find(db => db.period === currentPeriod);
            if (!db) {
                db = yield MultipleDatabase.createDatabase(currentPeriod);
            }
    ...
    delete() {
        return __awaiter(this, void 0, void 0, function* () {
            const dbs = yield this.getDbs();
            yield Promise.all(dbs.map(db => QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db.name).delete()));
        });
    }
    deleteOne(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const dbs = yield this.getDbsName();
            yield Promise.all(dbs.map(db => QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db).deleteOne(id)));
        });
    }
    find(id, forceFind = false) {
        return __awaiter(this, void 0, void 0, function* () {
            const dbs = yield this.getDbsName();
            const results = yield Promise.all(dbs.map(db => QueryBuilder.query(this.model, this.queryBuilder.getRelationships(), db).find(id, forceFind)));
    ...
    createIndex(index) {
        return __awaiter(this, void 0, void 0, function* () {
            const dbs = yield this.getDbsName();
            yield Promise.all(dbs.map(db => QueryBuilder.query(this.model, [], db).createIndex(index)));
        });
    }
}
